...
module Concerns::FullNameSplitter
  ...
  class Splitter
    ...
    def first_name
      first_name = @first_name.join(' ') unless @first_name.empty?
      first_name = first_name.to_s
      if first_name.last(2).include?('.') #omit middle initial
        return first_name.chomp(first_name.last(2)).strip
      end
      first_name
    end

    def last_name
      return @last_name.join(' ') unless @last_name.empty?
      ''
    end
